...
interface EmailModalProps {
  ...
  onSendGmail: (to: string, subject: string, content: string) => void;
  productKnowledge: ProductKnowledge | null;
  gmailConnected: boolean;
  gmailEmail: string | null;
  onConnectGmail: () => void;
  sendingGmail: boolean;
...
export default function EmailModal({
  isOpen, onClose, customerName, contactName, emails, generating,
  onRegenerate, onCopy, onSaveEmail, onSendGmail, productKnowledge,
  gmailConnected, gmailEmail, onConnectGmail, sendingGmail, customerEmail,
}: EmailModalProps) {
          ...
          <div className="px-6 py-4 border-t border-gray-100 space-y-3 flex-shrink-0">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 flex-1">
                {/* Copy */}
                <button onClick={() => onCopy(editedContent)} className="px-3 py-2 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 flex items-center gap-1.5 transition-colors">
                ...
                {/* Save to CRM */}
                <button onClick={() => onSaveEmail(parseInt(activeVersion.replace("version", "")), currentEmail.subject, editedContent)} className="px-3 py-2 text-sm font-medium bg-gray-700 text-white rounded-lg hover:bg-gray-800 flex items-center gap-1.5 transition-colors">
              ...
              {/* Gmail Send or Connect */}
              {gmailConnected ? (
                <button
                  onClick={() => onSendGmail(customerEmail || "", currentEmail.subject, editedContent)}
                  disabled={sendingGmail}
                  className="px-4 py-2 text-sm font-bold bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 flex items-center gap-1.5 transition-colors shadow-sm"
                >
                  {sendingGmail ? (
                    <>发送中...</>
                  ) : (
                ...
                <button onClick={onConnectGmail} className="px-4 py-2 text-sm font-medium border-2 border-red-200 text-red-600 rounded-lg hover:bg-red-50 flex items-center gap-1.5 transition-colors">
                  <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 1.636 0 0 1 0 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L5.455 4.64 12 9.548l6.545-4.91 1.528-1.145C21.69 2.28 24 3.434 24 5.457z"/></svg>
                  连接 Gmail 直接发送
                </button>
              )}

              {gmailConnected && gmailEmail && (
                <span className="text-xs text-green-600 ml-2 whitespace-nowrap">{gmailEmail}</span>
              )}
            </div>
            ...
            {gmailConnected && (
              <div className="text-xs text-gray-400 flex items-center gap-1">
                ...
                通过你的 Gmail 发送，每天限 20 封（防垃圾邮件），你始终可以审核后发送
              </div>
            )}
          </div>
        )}
